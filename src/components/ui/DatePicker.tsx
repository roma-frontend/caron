...
const WEEKDAYS = ['Երկ', 'Երք', 'Չրք', 'Հնգ', 'Ուր', 'Շբթ', 'Կիր']; // Monday-first
...
export function DatePicker({ value, onChange, placeholder = 'Ընտրել ամսաթիվ', className }: {
  value: string;
  ...
  const label = selected ? `${selected.d} ${MONTHS[selected.m]} ${selected.y}` : '';
  ...
  return (
    <>
      <button ref={triggerRef} type="button" onClick={() => setOpen((o) => !o)}
        ...
        {value && (
          <span role="button" tabIndex={0} aria-label="Մաքրել" onClick={(e) => { e.stopPropagation(); onChange(''); }}
      ...
      {open && typeof document !== 'undefined' && createPortal(
        <div ref={popRef} className="fixed z-[1000] rounded-xl border bg-popover p-3 shadow-2xl" style={{ top: pos.top, left: pos.left, width: CAL_W }}>
          ...
          <div className="mt-2 flex items-center justify-between border-t pt-2 text-xs">
            <button type="button" onClick={() => { onChange(''); setOpen(false); }} className="text-muted-foreground hover:text-destructive">Մաքրել</button>
            <button type="button" onClick={() => { const n = new Date(); onChange(toValue(n.getFullYear(), n.getMonth(), n.getDate())); setOpen(false); }} className="font-medium text-primary hover:underline">Այսօր</button>
          </div>
        </div>,
        document.body,
      )}
