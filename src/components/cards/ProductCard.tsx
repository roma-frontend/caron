...
export function ProductCard({ id, name, slug, price, compareAtPrice, image, category, inStock = true, stock, isNew, isHit, index = 0 }: ProductCardProps) {
      ...
      >
        ...
        {isHovered && (
          <div
            ...
            style={{ background: `radial-gradient(400px circle at ${mousePos.x}px ${mousePos.y}px, oklch(0.6 0.18 260 / 0.08), transparent 50%)`, filter: 'blur(30px)' }}
          />
        )}
        ...
        <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-muted/50 to-muted/30">
          ...
          {image ? (
            <Image src={image} alt={name} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
          ) : (
          ...
          {/* Discount badge */}
          {compareAtPrice && (
            <Badge className="absolute left-3 top-3 bg-destructive text-xs font-bold shadow-lg">
          ...
          {/* Favorite button */}
          <button
            onClick={(e) => { e.preventDefault(); toggleFav({ id, name, price, image: image ?? null }); (e.currentTarget.querySelector('svg') as unknown as HTMLElement)?.classList.add('heart-pulse'); setTimeout(() => (e.currentTarget.querySelector('svg') as unknown as HTMLElement)?.classList.remove('heart-pulse'), 400); }}
            className={`absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 ${isFav ? 'bg-red-500 text-white scale-110' : 'bg-white/80 text-muted-foreground hover:bg-white hover:text-red-500 hover:scale-110'}`}
          >
