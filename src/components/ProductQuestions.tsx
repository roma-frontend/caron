...
export function ProductQuestions({ productId }: { productId: Id<'products'> }) {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) { toast.error(t('sp.fillYourName')); return; }
    if (question.trim().length < 3) { toast.error(t('sp.writeYourQuestion')); return; }
    setSending(true);
    try {
      await ask({ productId, authorName: name, question, sessionToken: sessionToken || undefined });
      toast.success(t('sp.questionSubmitted'));
      setShowForm(false); setQuestion('');
    } catch { toast.error(t('sp.questionFailed')); } finally { setSending(false); }
  };
    ...
    <div className="mt-10">
      ...
      {showForm && (
        <Card className="mb-6">
          <CardContent className="p-5 space-y-3">
            ...
            <Textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder={t('sp.questionPlaceholder')} rows={3} />
            <Button onClick={handleSubmit} disabled={sending} className="gap-2">
              <Send className="h-4 w-4" /> {sending ? t('sp.sending') : t('sp.sendQuestion')}
            </Button>
          </CardContent>
        </Card>
      )}
      ...
      <div className="space-y-3">
            ...
            <CardContent className="p-4">
              ...
              {q.answer && (
                <div className="mt-3 flex gap-2 rounded-lg bg-muted/50 p-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-primary">{t('sp.shopAnswer')}</p>
        ...
        {questions?.length === 0 && !showForm && (
          <p className="py-6 text-center text-sm text-muted-foreground">{t('sp.noQuestions')}</p>
        )}
      </div>
    </div>
  );
