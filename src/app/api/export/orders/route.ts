...
export async function GET() {
  ...
  try {
    ...
    const rows = orders.map((o: Record<string, unknown>) =>
      `"${o.orderNumber}","${o.customerName}","${o.customerPhone}","${o.customerEmail}","${o.shippingAddress}",${o.total},"${o.status}","${o.paymentStatus}","${o.paymentMethod || ''}","${new Date(Number(o.createdAt)).toLocaleDateString()}"`
    ).join('\n');
