...
export default function CategoryPage() {
  ...
  const productCount = categoryCounts?.find(
  ...
  if (filters.brand)
    fchips.push({
      key: "brand",
      label: filters.brand,
      clear: () => setFilters({ ...filters, brand: undefined }),
    });
  if (filters.onSale)
    fchips.push({
      key: "sale",
  ...
  if (filters.minRating)
    fchips.push({
      ...
      clear: () => setFilters({ ...filters, minRating: undefined }),
    });
  if (filters.minPrice)
    fchips.push({
      key: "min",
  ...
  for (const [k, v] of Object.entries(filters.attributes ?? {})) {
    ...
    fchips.push({
      ...
      clear: () => {
        ...
        setFilters({
          ...filters,
          attributes: Object.keys(a).length ? a : undefined,
        });
      },
    });
  }
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            {category.imageUrl ? (
              <Image
                ...
                sizes="80px"
                priority
                unoptimized={category.imageUrl.startsWith("/api/")}
          ...
          <div className="mb-5 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-1 shrink-0 px-4 sm:px-0">
              <button
                ...
                className={`rounded-lg p-1.5 transition-colors ${viewMode === "grid" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent"}`}
                aria-label="Grid"
              >
              ...
              <button
                ...
                className={`rounded-lg p-1.5 transition-colors ${viewMode === "list" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent"}`}
                aria-label="List"
              >
            ...
            <div className="mb-5 flex flex-wrap items-center gap-2">
              {fchips.map((c) => (
                <button
                  key={c.key}
                  onClick={c.clear}
                  className="inline-flex items-center gap-1 rounded-full border bg-card px-3 py-1 text-xs transition-colors hover:border-primary/40 hover:text-primary"
                >
